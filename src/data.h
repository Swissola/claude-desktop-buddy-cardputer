...
static void _applyJson(const char* line, TamaState* out) {
  ...
  if (!la.isNull()) {
    // If the bridge sends more than 8 entries, skip the oldest so we always
    // display the most recent ones rather than the first ones.
    uint8_t total = (uint8_t)min((size_t)255, la.size());
